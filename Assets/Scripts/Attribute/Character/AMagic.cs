...
namespace Scripts
{
    public class AMagic:ABaseAttribute
    {
        ...
        public override void UpdateCurrentValue(float value)
        {
            ...
            if (value != 0)
            {
                EventCenter.Broadcast(Constants_Event.AttributeChange+":"+_gameData.Uid+":"+TypedAttribute.Mana);
                ...
                if (value > 0)
                {
                    _mCombatTextCreate.Type = SCTTYPE.Mana;
                }
                else
                {
                    _mCombatTextCreate.Type = SCTTYPE.ManaDamage;
                }
